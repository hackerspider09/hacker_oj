...
def run_python(containerId):
    ...
    # cmd  = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout 2s  python3 src/pyCode.py < src/input.txt'"
    # cmd  = f"docker exec container1 sh -c 'sudo docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout 2s  python3 src/pyCode.py < src/input.txt' '"
    cmd  = f"docker exec container sh -c 'docker exec {CONTAINER_NAME}{containerId}  'timeout 2s  python3 /src/pyCode.py < src/input.txt' '"
...
def run_cpp(containerId):
    ...
    if subprocessCppExe.returncode == 0:
        ...
        ExeCmd = f"sudo docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout 2s  src/./cppExeFile < src/input.txt'"
