...
def run_cpp(containerId):
    ...
    cmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'g++ src/cppCode.cpp -o src/cppExeFile'"  #3
    ...
    if subprocessCppExe.returncode == 0:
        ...
        ExeCmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout 2s  src/./cppExeFile < src/input.txt'"
...
def run_c(containerId):
    ...
    cmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'g++ src/cCode.c -o src/cExeFile'"
    ...
    if subprocessCExe.returncode == 0:
        ...
        ExeCmd = f"docker exec {CONTAINER_NAME} sh -c 'timeout 2s  src/./cExeFile < src/input.txt'"
        subprocessOutput = subprocess.Popen(
            ExeCmd,
            shell=True,
            # preexec_fn=setLimit(),
